...
/**
 * 用户使用 好友功能时必须进行注册，  服务器获取 用户注册信息之后 将 users表中的is_regist更改为1 表示该用户已经注册
 * 注意：得考虑 api安全，不能被随意调用并更改
...
public class RegistMassageServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		/*String key = request.getParameter("session");//该数据是将用户学号和时间进行加密 通过base64
